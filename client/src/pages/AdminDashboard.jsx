...
const AdminDashboard = () => {
    ...
    return (
        <div className="bg-background font-body text-white selection:bg-primary-fixed selection:text-on-primary-fixed min-h-screen">
            <Navbar />
            
            <div className="flex pt-16">
                <Sidebar isAdmin={true} />
                
                <main className="flex-1 ml-64 p-12 pt-24 bg-background">
                    <header className="mb-16">
                        <h1 className="font-headline text-6xl font-black tracking-tighter text-white mb-4">Command Center</h1>
                        <p className="font-body text-slate-400 text-lg max-w-3xl leading-relaxed">Oversee the academic ecosystem. Manage contributions, monitor platform health, and ensure knowledge integrity across all institutions.</p>
                    </header>

                    {/* Stats Grid */}
                    <section className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-16">
                        <div className="bg-white p-8 rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-slate-100 group hover:border-primary/20 transition-all text-slate-800">
                            ...
                            <div className="text-4xl font-headline font-black text-slate-900">248</div>
                            <div className="text-xs font-bold text-emerald-600 mt-3 flex items-center gap-1">
                                <TrendingUp size={14} /> +12% growth
                            </div>
                        </div>

                        <div className="bg-white p-8 rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-slate-100 group hover:border-secondary/20 transition-all text-slate-800">
                            <div className="flex justify-between items-start mb-6">
                            ...
                            <div className="text-4xl font-headline font-black text-slate-900">14</div>
                            <div className="text-xs font-bold text-slate-400 mt-3 uppercase tracking-widest">Currently live</div>
                        </div>

                        <div className="bg-white p-8 rounded-[2rem] shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-slate-100 col-span-1 md:col-span-2 relative overflow-hidden group text-slate-800">
                           <div className="relative z-10 flex flex-col justify-between h-full">
                                <div>
                                    ...
                                    <div className="text-3xl font-headline font-black mt-2 text-slate-900">Quantum Mechanics II</div>
                                </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <section className="lg:col-span-2 space-y-6">
                            <div className="flex items-center justify-between">
                                <h3 className="font-headline text-2xl font-bold flex items-center gap-3">
                                    Moderation Queue
                                    <span className="bg-red-100 text-red-600 px-2 py-0.5 rounded-lg text-xs font-label uppercase font-bold tracking-wider">{pendingNotes.length || 1} PENDING</span>
                                </h3>
                                <div className="flex items-center gap-4">
                                    <button 
                                        onClick={() => alert('All pending items will be reviewed by admin.')}
                                        className="bg-primary text-white px-4 py-1.5 rounded-lg text-xs font-label font-bold uppercase tracking-wider hover:opacity-90 transition-all flex items-center gap-2"
                                    >
                            ...
                            <div className="space-y-4">
                                {loading ? (
                                    ...
                                    /* Demo Moderation Queue Card */
                                    <div className="bg-white p-6 rounded-xl flex flex-col md:flex-row gap-6 items-center shadow-sm border border-slate-100 border-l-4 border-[#9d4300] text-slate-800">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-3 mb-1">
                                                <h4 className="font-bold text-lg leading-tight truncate text-slate-900">OOPS</h4>
                                                <span className="font-label text-[10px] bg-slate-100 px-2 py-0.5 rounded-full uppercase flex-shrink-0">PDF</span>
                                            </div>
                                            <div className="flex gap-4 mb-3 text-sm text-slate-500">
                                                <span className="font-medium truncate text-slate-700">Deepika Mishra</span>
                                                <span className="truncate">demo / CS</span>
                                            </div>
                                            <div className="bg-slate-50 p-3 rounded-lg text-sm italic text-slate-600 mb-2 border-l-2 border-slate-200 line-clamp-1">
                                                "College students notes"
                                            </div>
                                        </div>
                                        <div className="flex flex-row md:flex-col gap-2 w-full md:w-auto">
                                            <button 
                                                onClick={() => alert('Approved successfully')}
                                                className="flex-1 bg-primary text-white px-4 py-2 rounded-lg font-label text-[11px] font-bold uppercase hover:opacity-90 transition-all flex items-center justify-center gap-2 shadow-sm"
                                            >
                                                <CheckCircle size={14} /> Approve
                                            </button>
                                            <button 
                                                onClick={() => alert('Rejected successfully')}
                                                className="flex-1 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-lg font-label text-[11px] font-bold uppercase hover:bg-red-50 hover:text-red-600 transition-all flex items-center justify-center gap-2 shadow-sm"
                                            >
                                                <XCircle size={14} /> Reject
                                            </button>
                                            <button 
                                                onClick={() => alert('Comment added')}
                                                className="flex-1 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-lg font-label text-[11px] font-bold uppercase hover:bg-indigo-50 hover:text-indigo-600 transition-all flex items-center justify-center gap-2 shadow-sm"
                                            >
                                    ...
                                    displayedNotes.map(note => (
                                        <div key={note._id} className="bg-white p-6 rounded-xl flex flex-col md:flex-row gap-6 items-center shadow-sm border border-slate-100 border-l-4 border-secondary text-slate-800">
                                            <div className="w-16 h-20 bg-slate-50 rounded flex-shrink-0 flex items-center justify-center relative border border-slate-100">
                                                <FileText className="text-slate-300" size={32} />
                                                <div className="absolute bottom-1 right-1 w-4 h-4 bg-amber-500 rounded-full border-2 border-white"></div>
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-3 mb-1">
                                                    <h4 className="font-bold text-lg leading-tight truncate text-slate-900">{note.title}</h4>
                                                    <span className="font-label text-[10px] bg-slate-100 px-2 py-0.5 rounded-full uppercase flex-shrink-0">{note.fileType}</span>
                                                </div>
                                                <div className="flex gap-4 mb-3 text-sm text-slate-500">
                                                    <span className="font-medium truncate text-slate-700">{note.author}</span>
                        ...
                        <section className="space-y-8">
                            <div className="bg-white text-slate-800 p-6 rounded-xl border border-slate-100 shadow-sm">
                                <h3 className="font-headline text-xl font-bold mb-6 flex items-center gap-2 text-slate-900">
                                    <Flag size={20} className="text-red-500" />
                                    Reported Content
                                </h3>
                                <div className="space-y-4">
                                    {reports.map(report => (
                                        <div key={report._id || report.id} className="bg-slate-50 p-4 rounded-lg border-l-2 border-red-500 shadow-sm text-slate-800">
                                            <div className="text-xs font-label text-slate-400 mb-1 uppercase tracking-tighter font-bold">{report.type}</div>
                                            <div className="font-bold text-sm mb-2 text-slate-900">{report.note.title}</div>
                            ...
                            <div className="bg-white text-slate-800 p-6 rounded-xl border border-slate-100 shadow-sm">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="font-headline text-xl font-bold text-slate-900">New Contributors</h3>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="text-sm font-bold truncate text-slate-900">Brandon K.</div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="text-sm font-bold truncate text-slate-900">Maria Lopez</div>
                                ...
                                <button className="w-full mt-6 py-2 border border-slate-200 rounded-lg text-xs font-label font-bold uppercase text-slate-500 hover:bg-slate-50 hover:text-primary hover:border-primary transition-all shadow-sm">Manage All Users</button>
                            </div>
                        </section>
                    </div>
                </main>
            ...
            {/* Footer */}
            <footer className="bg-[#030816] border-t border-indigo-950/20 w-full py-12 px-8 mt-24 text-slate-400">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
                    <span className="font-headline font-bold text-primary text-lg">The Academic Curator</span>
                    ...
                    <div className="text-slate-500 text-sm">© 2024 The Academic Curator. Knowledge preserved.</div>
                </div>
            </footer>
        </div>
    );
